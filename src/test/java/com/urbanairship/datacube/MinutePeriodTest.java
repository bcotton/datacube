...
public class MinutePeriodTest {
    ...
    static class HostMinCube {
        Dimension<DateTime> time = new Dimension<DateTime>("time", new MinutePeriodBucketer(5), false, 8);
        ...
        public long getHostCountByMin(String hostName, DateTime dateTime) throws IOException, InterruptedException {
            Optional<LongOp> opt = dataCubeIo.get(new ReadBuilder(dataCube)
                .at(host, hostName)
                .at(time, dateTime));
            if(!opt.isPresent()) {
    ...
    public void test() throws Exception {
        HostMinCube hostMinCube = new HostMinCube();

        DateTime now = new DateTime(DateTimeZone.UTC);
        DateTime topOfHour = now.hourOfDay().roundFloorCopy();
        hostMinCube.addEvent("host1", topOfHour);
        hostMinCube.addEvent("host1", topOfHour.plusMinutes(6));
        hostMinCube.addEvent("host1", topOfHour.plusMinutes(6));
        hostMinCube.addEvent("host1", topOfHour.plusMinutes(6));

        Assert.assertEquals(1, hostMinCube.getHostCountByMin("host1", topOfHour));

        Assert.assertEquals(3, hostMinCube.getHostCountByMin("host1", topOfHour.plusMinutes(5)));
        Assert.assertEquals(0, hostMinCube.getHostCountByMin("host1", topOfHour.plusMinutes(10)));
